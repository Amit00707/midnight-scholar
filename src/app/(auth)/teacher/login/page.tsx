...
export default function TeacherLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setError(result.success ? 'This account is not a teacher account.' : result.error || 'Invalid credentials');
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-8 rounded-2xl border border-(--border) bg-(--surface) p-8 shadow-2xl">
        <div className="text-center">
          <h2 className="mt-6 text-3xl font-serif font-bold tracking-tight text-(--primary)">Teacher Sign In</h2>
          <p className="mt-2 text-sm text-(--muted)">Access teacher tools: classes, assignments, and analytics.</p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <p className="text-center text-sm text-(--muted)">Don't have an account? <Link href="/teacher/signup" className="font-medium text-(--primary) hover:text-amber-500">Sign up as teacher</Link></p>
        </form>
      </motion.div>
    </div>
  );
