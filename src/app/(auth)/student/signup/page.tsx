...
export default function StudentSignupPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const result = await signup(formData.name, formData.email, formData.password, 'student');
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md space-y-8 rounded-2xl border border-(--border) bg-(--surface) p-8 shadow-2xl">
        <div className="text-center">
          <h2 className="mt-6 text-3xl font-serif font-bold tracking-tight text-(--primary)">Create Student Account</h2>
          <p className="mt-2 text-sm text-(--muted)">Join as a student to access classes and assignments.</p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleRegister}>
          ...
          <p className="text-center text-sm text-(--muted)">Already a scholar? <Link href="/student/login" className="font-medium text-(--primary) hover:text-amber-500">Sign in</Link></p>
        </form>
      </motion.div>
    </div>
  );
